...
growth=CSV.read("./SMU_UA159/QC Analysis/OPMod_growth_database.csv",DataFrame)
...
CSV.write("./SMU_UA159/QC Analysis/OpMod_growth_database.csv",growth)
sum(growth[:,:PassAll])
sum(growth[:,:Pass5])
sum(growth[:,:Pass4])
sum(growth[:,:Pass3])
...
CSV.write("./SMU_UA159/QC Analysis/OpMod_rework.csv",rework)
...
data= CSV.read("./SMU_UA159/QC Analysis/OpMod_rework.csv",DataFrame)
...
experiment_name="SMU_UA159_OpMod_Rework"
...
SMU_data = CSV.read("./SMU_UA159/QC Analysis/OpMod_rework.csv",DataFrame)
...
CSV.write("./SMU_UA159/QC Analysis/Golden_Gate_Mantis/OpMod_Rework_Golden_Gate_Map.csv",dispense)
...
for i in plates
    ...
    plate=dispense[dispense[:,:Plate_ID].==i,1:14]
    colsums=[sum(plate[:,i]) for i= 1:14] .> 0 
    plate=plate[:,colsums]
